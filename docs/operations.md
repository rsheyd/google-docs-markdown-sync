...
For paired documents, both managed status footers surface the current failure compactly on their first line as **Needs attention** or **Temporarily paused** while retaining the last successful sync time. An unchanged error state remains stable across automatic retries without rewriting either footer, and a successful retry restores the normal footer title on both sides. Remote propagation is best-effort: a connectivity failure can be shown only in Markdown while Google Docs is unreachable. Detailed diagnostics remain in the service error log.
