...
OAuth and R2 credentials are stored in the macOS Keychain. The service, CLI, and Raycast share one explicit sync-location registry. The service loads indexed manifests directly, watches paired Markdown files, referenced image assets, and CSV directories locally, and polls the Google Drive changes feed every five seconds. Adding or explicitly rescanning a location discovers portable manifests beneath only that selected tree; routine polling performs no recursive location scan. Content is fetched when changes or enabled repair/conversion checks require it. Quiet polling uses the changes feed without checking every document. Daily reconciliation verifies all pairings in batches of at most 20, yielding to queued local edits and incremental remote targets between batches. Up to eight independent pairings run concurrently, while one coordinator commits shared state and notifications in stable order. API pressure uses bounded exponential backoff, and completed batches survive interruption. See the [scheduler design and measurements](docs/design/scalable-wake-safe-sync.md#september-2026-scheduler-implementation).
...
Markdown checkboxes (`- [ ]` / `- [x]`) round-trip as ordinary Docs bullets prefixed with `o] ` / `x] `. These easy-to-type text markers keep completion state accessible through the API without an add-on. Native checklist conversion is optional and off by default because Google does not expose reliable checked state; conversion may reopen completed tasks. See [checklist behavior and rationale](docs/formatting.md#checklists) and [the conversion toggle](docs/operations.md#native-checklist-conversion).

Google Docs' native Markdown export normally defines the supported Docs → Markdown subset. When Drive's export-size limit rejects a large document, GDMS falls back to serializing that subset through the Google Docs API. Headings, paragraphs, ordered and unordered lists, links, bold, italics, simple tables, blank-line spacing, heading links, and standalone images round-trip. Changed tables may contain one image per cell; rebuilt cell images use Google Docs' default size. Fixed Google Docs table-column widths round-trip through invisible, API-native point-width metadata immediately before each applicable Markdown table, and explicit within-cell breaks use inline `<br>` elements. Incremental pushes preserve unchanged document ranges and native tables of contents where the APIs allow it. Native Docs tables of contents appear locally as marked Markdown TOCs generated from headings; ordinary unmarked Markdown TOCs remain synchronized content. See the [formatting guide](docs/formatting.md#tables) and its [table-of-contents notes](docs/formatting.md#tables-of-contents).
...
- Mixed text-and-image paragraphs outside tables cannot be changed from Markdown; GDMS fails safely instead of risking image loss.
- A changed table structure may require a full Docs body rebuild. Cells with more than one image are refused; a supported cell image is reinserted at its text-relative position with the default Google Docs size.
